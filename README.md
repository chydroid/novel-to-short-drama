...
- **一键打包与范围导出**：导出中心把字幕 / 脚本 / 清单 / 数据 / 转码命令一次打成 .zip（可选连整轨配音），并支持「全部 / 仅已完成 / 跟随筛选 / 仅勾选章节 / 自定镜号」五种范围，导出前实时预览「几镜 / 多长 / 多大」，包内附打包说明
- **自定镜号表达式**：支持多段与排除（`1-20,25,30-35` 并集、`1-50,!8-10` 排除、`8-` / `-20` 开区间），写错的段自动忽略
- **剧集画面类型**：选「现代都市 / 古装年代 / 动画二次元」后，既按题材调评分阈值，也把对应画风写进生图 / 生视频提示词，避免古装被画成现代装、动画被画成实拍；并可「按类型套用整套配置」一键联动画风预设 + 全局画风令牌 + 提示词注入
- **批量断点续跑**：批量生图 / 视频 / 配音中途暂停后，剩余镜号**按类型分别**写进本机存档，三类可同时续跑；首页按类型列出「续跑」卡片，可「只跑剩余镜」直接按存档镜号投队列，也可「去定位」跳转
