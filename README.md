...
- **一键打包与范围导出**：导出中心把字幕 / 脚本 / 清单 / 数据 / 转码命令一次打成 .zip（可选连整轨配音），并支持「全部 / 仅已完成 / 跟随筛选 / 仅勾选章节 / 自定镜号」五种范围，导出前实时预览「几镜 / 多长 / 多大」（含**分文件体积明细**，zip 容器开销也计入），包内附打包说明
- **自定镜号表达式**：支持多段与排除（`1-20,25,30-35` 并集、`1-50,!8-10` 排除、`8-` / `-20` 开区间），**v2.20 起可用 `#章节名` 直接引用整章**（`1-100,!#第一集`），写错的段自动忽略；常用表达式可**存为命名预设**一键套用，预设按钮上标注**当前命中镜数**（v2.21）
- **剧集画面类型 + 套餐库**：选「现代都市 / 古装年代 / 动画二次元」后，既按题材调评分阈值，也把对应画风写进生图 / 生视频提示词；内置 3 套类型套餐（画风 + 全局令牌 + 提示词注入）一键套用，并支持**把当前配置存成自定义套餐**，可套用 / 删除
- **批量断点续跑**：批量生图 / 视频 / 配音中途暂停后，剩余镜号**按类型分别**写进本机存档，三类可同时续跑；首页按类型列出「续跑」卡片，可「只跑剩余镜」直接按存档镜号投队列、「去定位」跳转、「存为选区」把剩余镜号压成区间存成镜号预设（v2.21）、或**导出剩余清单**（CSV + BOM，Excel 直接开不乱码）
- **批量跑前预检**：生视频缺图片、配音缺台词的镜头在开跑前即被剔出，不占进度、不计失败，并明确提示跳过数量与原因
- **关键字段体检**：逐镜检查缺画面描述 / 无生图提示词 / 没时长 / 没章节 / 有台词没角色，分硬性软性两档；首页在有硬性缺料时给出体检卡，可「去修第一个」直接定位（v2.21）、查看明细或导出报告（只提示、不阻断批量）
