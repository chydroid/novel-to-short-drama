...
- **一键打包与范围导出**：导出中心把字幕 / 脚本 / 清单 / 数据 / 转码命令一次打成 .zip（可选连整轨配音），并支持「全部 / 仅已完成 / 跟随筛选 / 仅勾选章节」四种范围，包内附打包说明
- **剧集画面类型**：选「现代都市 / 古装年代 / 动画二次元」后，既按题材调评分阈值，也把对应画风写进生图 / 生视频提示词，避免古装被画成现代装、动画被画成实拍
