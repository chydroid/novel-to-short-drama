...
- **一键打包与范围导出**：导出中心把字幕 / 脚本 / 清单 / 数据 / 转码命令一次打成 .zip（可选连整轨配音，也可**附带体检报告 / 剩余清单**），并支持「全部 / 仅已完成 / 跟随筛选 / 仅勾选章节 / 自定镜号」五种范围，导出前实时预览「几镜 / 多长 / 多大」（含**分文件体积明细**与整轨压缩建议，zip 容器开销也计入），包内附打包说明；v2.22 修掉了自写 zip 的多文件偏移错误（此前第 2 个文件起在解压软件里会报错）
- **自定镜号表达式**：支持多段与排除（`1-20,25,30-35` 并集、`1-50,!8-10` 排除、`8-` / `-20` 开区间），**v2.20 起可用 `#章节名` 直接引用整章**（`1-100,!#第一集`），**v2.22 起可用 `@预设名` 引用别的预设**（`1-20,@片头,!@片尾`），写错的段自动忽略；常用表达式可**存为命名预设**一键套用、可**上移排序**，预设按钮上标注**当前命中镜数**（v2.21）并可点「筛」直接到生成页只看这几个镜（v2.22）
...
- **批量断点续跑**：批量生图 / 视频 / 配音中途暂停后，剩余镜号**按类型分别**写进本机存档，三类可同时续跑；首页按类型列出「续跑」卡片，可「只跑剩余镜」直接按存档镜号投队列、「去定位」跳转、「存为选区」把剩余镜号压成区间存成镜号预设（v2.21）、或**导出剩余清单**（CSV + BOM，Excel 直接开不乱码）；生图与生视频同时有剩余时可一键**「图→视频 串跑」**（先补图 → 自动接生视频，v2.22）
...
- **自带 xlsx 导出**：xlsx 本质是 zip + 几段 XML，复用自写的 zip 实现**零外部依赖**生成工作簿（数字走数值单元格，可排序）；体检报告与剩余清单都能导成 .xlsx，剩余清单一类一张工作表（v2.22）
